...
export function DuctPanel() {
  ...
  return (
    <div>
      ...
      <div>
        <span>断面形状</span>
        <span>{node.shape === 'rectangular' ? '矩形' : '円形'}</span>
      </div>
      ...
      <div>
        <span>風量 (m³/h)</span>
        <span>{node.airflowRate != null ? node.airflowRate : '未計算'}</span>
      </div>

      {/* 【圧損】: 読み取り専用（計算結果） */}
      <div>
        <span>圧損 (Pa)</span>
        <span>{node.calcResult != null ? node.calcResult.totalPressureLoss : '未計算'}</span>
      </div>
      ...
      <div>
        <span>始端ポート</span>
        <span>{node.startPortId}</span>
      </div>
      <div>
        <span>終端ポート</span>
        <span>{node.endPortId}</span>
      </div>
    </div>
  )
}
