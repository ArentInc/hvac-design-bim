...
export const updateNodesAction = (
  ...
  for (const u of updates) idsToMarkDirty.add(u.id)
  for (const pId of parentsToUpdate) idsToMarkDirty.add(pId)

  // Add to pending updates set
  for (const id of idsToMarkDirty) pendingUpdates.add(id)
